<template>
  <transition name="modal">
    ...
    <div class="wrap popup_layout">
              ...
              <table class="tbl">
                ...
                <tbody>
                  <tr v-for="(data, index) in storeList" :key="index">
                    <th scope="row" @click="clickStore(data.VDCD, data.VDSNM, data.BRCD, data.SUCD)">
                      <span class="num_box">
                        <strong>{{index+1}}</strong>
                      </span>
                      <label>{{data.VDSNM}}</label>
                    </th>
                    <!-- 합계 -->
      ...
      <store-bydt-popup
        v-if="isStoreByDtVisible"
        @close="closeStoreByDt"
        :dr_STORE="dr_STORE"
...
import storeBydtPopup from '@/pages/financial/FIN0104'
...
export default {
  ...
  methods: {
    ...
    getStoreList() {
      ...
      // 매장 매출순위 팝업 리스트 개수
      // 당일매출 순위
      let sql = "SELECT ROWNUM() RN, VDCD, VDSNM, SUCD, BRCD, SALE_TOT, QTY_TOT, "
      sql += "JAMT, JQTY, DCAMT, DCQTY, GAMT, GQTY, "
      sql += "R_JAMT, R_JQTY, R_DCAMT, R_DCQTY, R_GAMT, R_GQTY, "
      sql += "ADVDEPAMT FROM ( "
      sql += "SELECT VDCD, VDSNM, SUCD, BRCD, SUM(JAMT+DCAMT+GAMT+ADVDEPAMT) AS SALE_TOT, SUM(JQTY+DCQTY+GQTY) AS QTY_TOT, "
      ...
      sql += "GROUP BY VDCD, VDSNM, SUCD, BRCD "
    ...
    clickStore(vdcd, vdnm, brcd, sucd) {
      console.log("매장!!!! >> ", vdcd, " / ", vdnm, " / ", sucd)
      console.log(typeof sucd)
      console.log(sucd == '12')
      console.log(sucd == "12")
      console.log(sucd == 12)
      let param_brcd = '';
      ...
      this.dr_STORE = {VDCD: vdcd, VDNM: vdnm, BRCD: param_brcd};
      this.showStoreByDt();
    },
    showStoreByDt() {
      ...
      this.isStoreByDtVisible = true;
    },
    closeStoreByDt() {
      this.isStoreByDtVisible = false;
    },
  },
  filters: {
