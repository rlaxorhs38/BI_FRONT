<template>
  <div class="wrap">
                        ...
                        <table class="tbl">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">연락처</th>
                                    <th v-if="selectedCOMPANYCD == 3" scope="col">사업부
                                      <!-- <span class="tooltip">
                                      ...
                                      <label class="tbl_dropdown" for="tbl-toogle-b1" v-show="selectedCOMPANYCD == 3">
                                    ...
                                    <th v-else-if="selectedCOMPANYCD == 4" scope="col">브랜드</th>
                                    <th scope="col">매장형태
                                    ...
                                    <th scope="col">유통사
                                      <label class="tbl_dropdown" for="tbl-toogle-ch" v-show="selectedCOMPANYCD == 3">
                        ...
                        <table class="tbl tbl_center">
                            ...
                            <tbody>
                                ...
                                <tr v-for="(data , index) in SDList" :key="data.HRID" class="watermark" :style="{'background-image':Bg}">
                                    ...
                                    <td class="tl" style="cursor:pointer" @click="selectedCOMPANYCD == '4' ? toOtherSeDetail(data.VDCD, data.SNO, data.BRCD, data.SUCD, data.HRID) : toSeDetail(data.VDCD, data.SNO, data.BRCD, data.SUCD, data.HRID)">
                                    ...
                                    <td>{{ data.PHONE }}</td>
                                    <td v-if="selectedCOMPANYCD == 3">{{ data.SUNM }}</td>
                                    <td v-else-if="selectedCOMPANYCD == 4">{{ data.BRNM }}</td>
                                    <td>{{ data.CHGUNM }}</td>
                                    <td>{{ data.CHGUCOM }}</td>
                                    <td v-if="selectedCOMPANYCD == 3">{{ data.STORENM }}</td>
                                    <td v-else-if="selectedCOMPANYCD == 4">{{ data.VDSNM }}</td>
                                    <td v-if="selectedCOMPANYCD == 3" class="tr">{{ data.TWOYEAR_AGO_AMT }} <span v-show="data.TWOYEAR_AGO_AMT">억</span></td>
                                    <td v-else-if="selectedCOMPANYCD == 4" class="tr">{{ data.TWOYEAR_AGO_AMT }} <span v-show="data.TWOYEAR_AGO_AMT">억</span></td>
                                    <td v-if="selectedCOMPANYCD == 3" class="tr">{{ data.ONEYEAR_AGO_AMT }} <span v-show="data.ONEYEAR_AGO_AMT">억</span></td>
                                    <td v-else-if="selectedCOMPANYCD == 4" class="tr">{{ data.ONEYEAR_AGO_AMT }} <span v-show="data.ONEYEAR_AGO_AMT">억</span></td>
                                    <td class="tooltip">
    ...
    <seDetailPopup
      v-if="isSeDetailPopup"
      :data="detailPopupData"
      @close="closeListPopup"
    ></seDetailPopup>
    <otherSeDetailPopup
      v-if="isOtherSeDetailPopup"
...
import seDetailPopup from '@/pages/hum/HUM0401'
import otherSeDetailPopup from '@/pages/hum/HUM0501'
...
export default {
  ...
  computed: {
    ...
    // 자사,타사
    COMPANYList() {
      return this.$store.getters.getSECOMPANYCDList
    },
    // 브랜드 리스트
    getBRCDList() {
  ...
  data() {
    return {
      ...
      date: "",
      makeDataDate: null,
      isSeDetailPopup: false,
      isOtherSeDetailPopup: false,
  ...
  methods: {
    ...
    getSDList() {
      ...
      if(this.selectedSi.length==0 || this.selectedGu.length==0 || (this.selectedCOMPANYCD==3 && this.selectedBrand.length==0)  
        || this.selectedAmtRating.length==0 || this.selectedCHGU.length==0){
        return
      }
    ...
    toSeDetail(VDCD,SNO,BRCD,SUCD,HRID){
      this.waterMark = false
      this.detailPopupData = {VDCD: VDCD, SNO: SNO, BRCD:BRCD, SUCD:SUCD, HRID:HRID}
      this.isSeDetailPopup = true
    },
    toOtherSeDetail(VDCD,SNO,BRCD,SUCD,HRID){
      this.waterMark = false
      this.otherDetailPopupData = {VDCD: VDCD, SNO: SNO, BRCD:BRCD, SUCD:SUCD, HRID:HRID}
      this.isOtherSeDetailPopup = true
    },
    closeListPopup() {
      this.isSdSalesListPopup = false
      this.isSeDetailPopup = false
      this.isOtherSeDetailPopup = false
      this.waterMark = true
    },
    showYN(){
