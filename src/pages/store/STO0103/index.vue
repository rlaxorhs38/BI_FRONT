    ...
    <header class="header">
      <div class="header_inner">
        ...
        <div class="groups">
          ...
          <!-- select -->
          <div v-show="choice == 3" class="select_box" style="min-width:150px;">
            <select class="select" v-model="year" @change="chageDate">
              <option v-for="data in y_options" :key="data" :value="data">{{ data }}년</option>
            </select>
          </div>
          <!-- select -->
          <div v-show="choice == 2" class="select_box" style="min-width:150px;">
            <select class="select" v-model="year" @change="chageDate">
              <option v-for="data in y_options" :key="data" :value="data">{{ data }}년</option>
            </select>
          </div>
        ...
        <div class="list_box">
          <ul class="list_card">
            <li>
              ...
              <div class="card card_mini">
                <div class="card_title">
                  <h4 class="card_title_text">누적 매출합계</h4>
                </div>
                <div class="card_content">
                  ...
                  <dl class="list_obj">
                    ...
                    <dt class="tit" v-if="selectedCODE == '1'">선수금</dt>
                    <dd class="txt" v-if="selectedCODE == '1'">{{Math.round(Number(dr_H.ADVDEPAMT)/1000000) | currency}}</dd>
                    <!-- <dt class="tit" v-if="(((selectedCODE == '1') || (selectedCODE == 'MI')) && !selectStoreSU) || (selectStoreSU == '1')">선수금</dt>
                    <dd class="txt" v-if="(((selectedCODE == '1') || (selectedCODE == 'MI')) && !selectStoreSU) || (selectStoreSU == '1')">{{Math.round(Number(dr_H.ADVDEPAMT)/1000000) | currency}}</dd> -->
                  </dl>
                </div>
              </div>
            </li>
            <li>
              ...
              <div class="card card_mini">
                <div class="card_title">
                  <h4 class="card_title_text">누적 매출 달성률</h4><small class="txt_box">(누적 매출합계/목표금액) * 100</small>
                </div>
                <div class="card_content">
                  <strong class="em_obj">
            ...
            <li>
              ...
              <div class="card card_mini">
                <div class="card_title">
                  <h4 class="card_title_text">누적 평균 할인율</h4><small class="txt_box">(1-(제품 매출금액/TAG금액) * 100)</small>
                </div>
                <div class="card_content">
          ...
          <div class="col_md_6">
            <div class="cont_box">
              <div class="tit">
                <div class="tab">
                  <ul>
                    <li :class="{on: salesTab == 0}"><a href="javascript:void(0);" @click="chageSalesTab(0)">누적 매출비교</a></li>
                ...
                <div class="graph_area">
                  ...
                  <div class="graph_view" style="width:50%;">
                    ...
                    <div class="graph" style="max-width:340px; height:280px;">
                        <div id="STO0103_chartdiv1" style="position:relative; width:100%; height:100%; float:left;"></div>
                      </div>
                      <!-- 카테고리 -->
                  ...
                  <div class="graph_view" style="width:50%;">
                    ...
                    <div class="graph" style="max-width:280px; height:280px;">
                        <div id="STO0103_chartdiv2" style="position:relative; width:100%; height:100%; float:left;"></div>
                      </div>
                      <!-- 카테고리 -->
                      <div class="graph_category">
                        <ul>
                  ...
                  <div class="graph_area">
                    <div class="graph_view">
                      ...
                      <div class="graph" style="max-width:870px; height:400px;">
                        <div id="STO0103_chartdiv3" style="position:relative; width:100%; height:100%; float:left;"></div>
                      </div>
                    </div>
                  </div>
                </div> 
                      ...
                      <div class="graph_area">
                        <div class="graph_view">
                          ...
                          <div style="position:relative; width:100%; height:170px; float:left;">
                            <div id="STO0103_chartdiv4" style="position:relative; width:100%; height:100%;"></div>
                          </div>
                          </div>
                          <!-- 카테고리 -->
                          <div class="graph_category">
                    ...
                    <div class="col_md_6 flex_c">
                      <div class="graph_view">
                        <div class="graph_cont">
                          ...
                          <strong class="graph line_bar">
                            <!-- <span class="bar bar_sty01" :style="{width:nanToZero(rate(dr_H.TQTY, dr_H.INQTY)) +'%'}"></span> -->
                      ...
                      <div class="graph_cont">                          
                          ...
                          <strong class="graph line_bar">
                            <span
                            class="bar bar_sty02"
                            :style="{width:(Math.round((1 - (Number(dr_H.TSAMT))/Number(dr_H.TTAGPRI)) * 100)) +'%'}"
                          ></span>
                          </strong>
                      </div>
                      <div class="graph_cont">                          
                          ...
                          <strong class="graph line_bar">
                            <span
                            class="bar bar_sty03"
                            :style="{width:((Math.abs(Number(dr_H.RQTY))/Number(dr_H.SQTY)) * 100) +'%'}"
                          ></span>
                          </strong>
                      </div>
                    </div>
...
export default {
  ...
  methods: {
    ...
    getHeaderData() {
      this.req2svr.getHeaderData(this.tabType, this.selectedCODE, this.year, this.store).then(
        res => {
          this.dr_H = {}
          if (res.MACHBASE_ERROR) {
            console.log("res", res)
    ...
    getSalesChartData() {
      this.req2svr.getSalesChartData(this.tabType, this.selectedCODE, this.year, this.store).then(
        res => {
    ...
    getSalesLineCharData() {
      this.req2svr.getSalesLineCharData(this.tabType, this.selectedCODE, this.year, this.store).then(
        res => {
          this.dr_P = []
          if (res.MACHBASE_ERROR) {
            console.log("res", res)
            ...
            let _LTSAMT = 0, _TSAMT = 0
            for (let i=0;i<list.length;i++) {
              this.dr_P.push({
                SALEMM: Number(list[i].SALEMM) + "월",
                VALUE1: Math.round((_LTSAMT + Number(list[i].LTSAMT))/1000000),
                VALUE2: Math.round((_TSAMT + Number(list[i].TSAMT))/1000000),
              })
              _LTSAMT += Number(list[i].LTSAMT)
              _TSAMT += Number(list[i].TSAMT)
            }
          }
          this.makeSalesLineChart()
        },
    ...
    getSalesAnalysisData() {
      this.req2svr.getSalesAnalysisData(this.tabType, this.selectedCODE, this.year, this.store).then(
        res => {
          this.dr_C = {}
          if (res.MACHBASE_ERROR) {
            console.log("res", res)
    ...
    makeSalesLineChart() {
      ...
      AmCharts.makeChart("STO0103_chartdiv3", {
        type: "serial",
    ...
    makeSalesChart() {
      let data = [
        {category: "전년누적", value: Math.round(Number(this.dr_S.LTSAMT)/1000000), color: Number(this.dr_S.LTSAMT) < 0 ? "#fbccca" : "#c5defc"},
        {category: "당년목표", value: Math.round(Number(this.dr_S.GAMT)/1000000), color: Number(this.dr_S.GAMT) < 0 ? "#c2c2c3" : "#c2c2c3"},
        {category: "당년누적", value: Math.round((Number(this.dr_S.TSAMT)+Number(this.dr_S.ADVDEPAMT))/1000000), color: Number(this.dr_S.TSAMT)+Number(this.dr_S.ADVDEPAMT) < 0 ? "#f1574e" : "#3f93f5"}        
      ]

      AmCharts.makeChart("STO0103_chartdiv1", {
        type: "serial",
        startEffect: "easeOutSine",
    ...
    makeSalesPercentChart() {
      let date = this.year + "1201"
      let data = [
        { category: moment(date).subtract(1, "years").format("YYYY"),
        ...
        { category: moment(date).format("YYYY"),
      ...
      AmCharts.makeChart("STO0103_chartdiv2", {
        type: "serial",
        startEffect: "easeOutSine",
        categoryField: "category",
        columnSpacing: 0,
    ...
    makeSalesAnalysisChart() {
      ...
      AmCharts.makeChart("STO0103_chartdiv4", {
        type: "serial",
        startEffect: "easeOutSine",
        categoryField: "category",
        rotate: true,
